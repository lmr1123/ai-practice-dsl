...
const scenarioConfig = {
  member: {
    ...
    goals: ["自然开口", "讲清活动权益", "回应手机号隐私顾虑", "拒绝后礼貌收尾"],
    required: ["免费办理", "会员价或新人券", "积分或会员日", "手机号用途", "不强迫", "正常结账"],
    requiredChecks: [
      { key: "免费办理", terms: ["免费", "免费办理", "不收费"], points: 15 },
      { key: "活动权益", terms: ["本单", "这单", "当次", "次日", "下次", "会员价", "新人券", "优惠券", "券", "优惠", "积分"], points: 25 },
...
function fallbackScore(text, scenarioId) {
  ...
  return {
    customer_reply: score >= 80
      ...
      : (scenarioId === "member" ? "你还没说清楚现在办有什么用？券是今天用还是下次用？手机号会怎么用？" : "你还没问我家里人的情况，老人有高血压也能一起买吗？"),
    score,
    hit_points: hitPoints,
    risk_points: riskPoints,
    polished_reply: scenarioId === "member"
      ? "理解，您赶时间我尽快给您结。会员不是强制的，不办也能正常买。我例行跟您说一下：现在办是免费的，权益按门店活动可能有会员价、当次或下次可用券，也能累计积分、参加会员日。手机号主要用于账户、积分和券到账；您不方便的话，我就先给您正常结账。"
  ...
  const messages = [
    {
      ...
      content: [
        ...
        "收银台办卡场景不要默认承诺本单一定立减。新人券、会员价、积分、会员日等权益必须按门店活动和系统到账规则表达；如果员工没有说明规则，就把它作为改进点。",
        "润色表达要像真实药店收银台口语：先安抚赶时间，再说明免费、不强制、活动权益、手机号用途，最后给顾客选择权。"
      ].join("\n")
    },
    {
      role: "user",
